;;; user-machine -- Machine-specific initialisation script.

;;; Commentary:

;; Attempts to load a machine-specific configuration file.
;;
;; If no machine-specific configuration is found, a default configuration for
;; unknown machines is loaded.

;;; Code:

(defun user-machine-init ()
  "Return an interned symbol of the machine-specific initialisation module."
  (intern (downcase (format "user-machine-%s"
                            (car (split-string (system-name) "\\."))))))

;; Attempt to include the machine-specific configuration file, and if that
;; fails fall back to the unknown machine configuration.
(condition-case error
    (require (user-machine-init))
  (file-missing
   (warn "No machine specific configuration '%s'" (user-machine-init))
   (require 'user-machine-unknown)))

(provide 'user-machine)
;;; user-machine.el ends here
